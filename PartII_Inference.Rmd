...
We fit the test data to the model:
```{r}
...
test$migrantprogram <- test$migrantprogram + .5
test$english_learner <- test$english_learner + .5
rmse.test.fm6 <- sqrt(mean((test$result.max - predict(fm6, test))^2))
sprintf("test rmse: prediction model %s", (rmse.test.fm6))
```
...
boot.out <- boot(train.regression, coefficient.boot, 10000)
