...
rmse.train.6 = sqrt(mean((exp(fm6$residuals))^2))
sprintf("training rmse: prediction model %s", rmse.train.6)
...
rmse.test.fm6 <- mean((test$y - predict(fm6, test))^2)
...
best_aic_model = glm(action_level_exceedance ~ charter_school + unduplicatedfrpm_eligible_count + 
    english_learner, family = binomial(), data = train)
```
...
threshold2 <- 0.02
table(fitted_model > threshold2, train$action_level_exceedance)
